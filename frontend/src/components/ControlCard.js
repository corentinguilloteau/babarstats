	...
	switchComponents(state, idx) {
		switch (state.type) {
				...
				return (
					<div className="form-group mb-3">
						<input
							...
							defaultChecked={
								this.props.sensor[state.name] ? "checked" : ""
							}
						/>
						<label
							htmlFor={"state" + idx}
			...
			case "range":
				return (
					<div className="form-group mb-3">
						<label htmlFor={"state" + idx} className="form-label">
							{state.displayedname}
						</label>
						<input
							index={idx}
							...
							readOnly={!state.editable}
							defaultValue={this.props.sensor[state.name]}
						/>
					</div>
				);
			default:
			...
			<div className="col-12 col-sm-6 col-xxl-3 d-flex">
				<div className="card">
					...
					<div className="card-body py-2">
						<form>
							{Object.values(this.props.states[0]).map(
								(item, idx) => {
